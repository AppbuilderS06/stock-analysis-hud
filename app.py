...
def render_disclaimer():
    ...
    st.markdown(
        '<div style="text-align:center;font-size:10px;color:#374151;padding:8px 0;letter-spacing:1px;">'
...
def render_hud():
    ...
    # ── Timeframe score computation ───────────────────────
    market_ctx_raw = st.session_state.get('market_ctx', {})
    mkt_phase      = get_market_phase()
    sec_phase_num  = st.session_state.get('phase_result', (0,))[0]
    market_ctx_raw['mkt_phase'] = mkt_phase[0]
    market_ctx_raw['sec_phase'] = st.session_state.get('sec_phase_num', 0)

    tf_scores = calc_timeframe_scores(
    ...
    with c2:
        ...
        <div class="score-card">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;">
            <div>
              <div style="font-size:11px;color:#818CF8;letter-spacing:2px;
        ...
        if sector_etf_sc:
            ...
            # Compute returns for selected window
            def _ret(series, tf):
                try:
                    s = series.dropna()
                    ...
                    if tf == '1W':
                        ...
                        mask = s.index >= q_start.replace(tzinfo=s.index.tzinfo) if hasattr(s.index, 'tzinfo') else s.index >= q_start
                        past = s[mask].iloc[0] if mask.any() else s.iloc[-66]
                    elif tf == 'YTD':
                        ytd_start = datetime(now.year, 1, 1)
                        mask = s.index >= ytd_start.replace(tzinfo=s.index.tzinfo) if hasattr(s.index, 'tzinfo') else s.index >= ytd_start
                        past = s[mask].iloc[0] if mask.any() else s.iloc[0]
                    elif tf == '1Y':
                        past = s.iloc[-252] if len(s) >= 252 else s.iloc[0]
                    else:
                        past = s.iloc[-22]
                    return float((s.iloc[-1] / past - 1) * 100)
                except: return 0.0

            # Fetch comparison data
            try:
                comp_df = yf.download(
                    [sector_etf_sc, 'SPY'], period='2y',
                    auto_adjust=True, progress=False, threads=True
                )
                spy_close = comp_df['Close']['SPY'].dropna()
                sec_close = comp_df['Close'][sector_etf_sc].dropna()
            except:
                spy_close = None
                sec_close = None

            # Stock return from df
            stk_ret = _ret(df['Close'], perf_tf)
            spy_ret = _ret(spy_close, perf_tf) if spy_close is not None else 0.0
            sec_ret = _ret(sec_close, perf_tf) if sec_close is not None else 0.0
            ...
            <div style="background:#070F1A;border:1px solid #1A2A3A;border-radius:8px;
              ...
              <div style="margin-top:8px;padding-top:6px;border-top:1px solid #1A2A3A;
                          font-size:10px;color:{rel_col_sc};font-weight:600;">{rel_msg_sc}</div>
            </div>""", unsafe_allow_html=True)
    ...
    with vc2:
        ...
        bb_rows += (
            ...
            '</div>'
            f'<div style="text-align:center;font-size:11px;color:#94A3B8;">{cur}{cur_close:.2f} · Mid {cur}{bb_mid:.2f}</div>'
            '</div>'
        )
    ...
    <div style="background:#1A2232;border:1px solid #243348;border-radius:0 0 8px 8px;padding:12px 16px 10px;">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        <div style="font-size:12px;color:#94A3B8;">Pre-filled from AI analysis · Adjust any value to recalculate</div>
    ...
    if not insider_data:
        ...
        if sell_val > 0 and sell_val > buy_val * 2:
            ...
            st.markdown(f'''<div style="background:#1A0505;border:1px solid #FF6B6B66;border-radius:8px;
              ...
              <span style="font-size:15px;">⚠️</span>
              <div>
                <span style="color:#FF6B6B;font-weight:800;font-size:12px;
                  font-family:'JetBrains Mono',monospace;">INSIDER SELLING DOMINATES</span>
                <span style="color:#FCA5A5;font-size:11px;margin-left:10px;">
            ...
            st.markdown(f'''<div style="background:#030F07;border:1px solid #00FF8844;border-radius:8px;
              ...
              <div>
                <span style="color:#00FF88;font-weight:800;font-size:12px;
                  font-family:'JetBrains Mono',monospace;">INSIDER BUYING ACTIVE</span>
                <span style="color:#86EFAC;font-size:11px;margin-left:10px;">
            ...
            with cols[i]:
                ...
                <div class="{pcls}">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:4px;">
                    ...
                    <a href="{inv_url}" target="_blank" style="font-size:10px;color:#4A6080;text-decoration:none;">ⓘ</a>
                  </div>
            ...
            if not results:
                ...
                for r in results[:15]:
                    ...
                    <div style="background:#1A2232;border:1px solid #243348;border-radius:8px;
                      ...
                      <div style="flex:1;min-width:0;">
                        <div style="font-size:12px;font-weight:600;color:#E2E8F0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{r["name"]}</div>
                      ...
                      <div style="text-align:center;min-width:60px;">
                        <div style="font-size:13px;color:#A78BFA;font-family:monospace;font-weight:700;">{r["rsi"]:.0f}</div>
